...
public partial class Default2 : System.Web.UI.Page
{
    ...
    {
        tabla = new DataTable();
        tabla = servicio.Consultarbasedatos(string.Format("select cod_paquete,estatus,categoria from Infopaquetes where cod_casilla={0}",TextBox14.Text));
    ...
    protected void Button6_Click1(object sender, EventArgs e)
    {
        for (int i = 0; i < GridView2.Rows.Count; i++)
        {
            ...
            if (ch.Checked == true)
            {
                ...
                datos = servicio.Consultarbasedatos(string.Format("select * from Paquete where cod_paquete={0}", uno));
            }
        }
        DetailsView1.DataSource=datos;
